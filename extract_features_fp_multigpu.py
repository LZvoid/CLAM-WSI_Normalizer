...
def batch_normalize_efficient(batch, normalizer):
	...
	批量标准化处理 - 已移除，不再使用标准化
	"""
	# 标准化功能已移除，直接返回原始batch
	print("Warning: batch_normalize_efficient called but normalization is disabled")
	return batch

def preload_normalizer(device, target_path="target_imgs/her2_optimal_target_512x512_level0_h5.png"):
	"""
	预加载和预热normalizer - 已移除，不再使用标准化
	"""
	# 标准化功能已移除，直接返回None
	print(f"GPU {device}: Normalization disabled - proceeding with direct feature extraction")
	return None
...
class OptimizedDataLoader:
	...
	"""
def compute_w_loader_optimized(output_path, loader, model, verbose=0, gpu_id=0):
	"""
	优化的特征提取函数，不使用标准化
	args:
		output_path: directory to save computed features (.h5 file)
	...
	for count, data in enumerate(tqdm(loader, desc=f'GPU {gpu_id}')):
		with torch.inference_mode():
			...
			try:
				...
				if verbose > 0:
					print(f"GPU {gpu_id}: Feature extraction failed for batch {count}: {e}")
				
				# 创建零特征作为fallback
				batch_size = len(batch)
				feature_dim = 1024 if 'resnet50' in str(model) else 512
	...
	print(f"Failed feature extractions: {failed_patches}")
...
def process_single_wsi(wsi_info, args, gpu_id, results_queue, progress_queue):
    ...
    try:
        ...
        slide_id = str(slide_id)  # ✅ 保证纯Python类型
        ...
        # 移除标准化器初始化，直接进行特征提取
        print(f"GPU {gpu_id}: Model loaded, proceeding without normalization")
        ...
        class OptimizedDataLoader:
            def __init__(self, dataset, batch_size, num_workers=0, prefetch_factor=2):
        ...
        def compute_w_loader_optimized(output_path, loader, model, gpu_id=0):
            device = torch.device(f'cuda:{gpu_id}')
            model = model.to(device)
            mode = 'w'
            features_cache = []
            ...
            for data in loader:
                with torch.inference_mode():
                    ...
                    try:
                        ...
                        # 创建零特征作为fallback
                        batch_size = len(batch)
                        feature_dim = 1024 if 'resnet50' in args.model_name else 512
                        features = np.zeros((batch_size, feature_dim), dtype=np.float32)
                    ...
                    if cache_size >= max_cache_size:
                        combined_features = np.vstack(features_cache)
                        combined_coords = np.vstack(coords_cache)
                        save_hdf5(output_path, {'features': combined_features, 'coords': combined_coords}, None, mode)
                        mode = 'a'
                        features_cache, coords_cache, cache_size = [], [], 0
        ...
        output_file_path, failure_percentage = compute_w_loader_optimized(
            output_path, optimized_loader, model, gpu_id
        )
        ...
        torch.save(torch.from_numpy(features), os.path.join(args.feat_dir, 'pt_files', slide_id + '.pt'))
        torch.cuda.empty_cache()

        result = {
            'slide_id': str(slide_id),
            'failure_percentage': float(failure_percentage),
            'processing_time': float(time_elapsed),
            'gpu_id': int(gpu_id)
        }
...
def compute_w_loader_batch_parallel(output_path, loader, model, verbose=0, num_gpus=1):
	"""
	批次级并行的优化计算函数，不使用标准化
	"""
	...
	for count, data in enumerate(tqdm(loader)):
		with torch.inference_mode():
			...
			try:
				...
				failed_patches += len(batch)
				if verbose > 0:
					print(f"Feature extraction failed for batch {count}: {e}")
...
if __name__ == '__main__':
	...
	if args.data_parallel_mode == 'wsi' and num_gpus > 1:
		...
		# 不使用标准化器，直接进行特征提取
		print("Proceeding without normalization - direct feature extraction only")
		...
		for bag_candidate_idx in tqdm(range(total)):
			...
			try:
				...
				# 使用优化的compute_w_loader函数，不使用标准化
				...
				print(f'Feature extraction failure rate for {slide_id}: {failure_percentage:.2f}%')
	...
	# 保存统计信息到文件
	statistics_file = os.path.join(args.feat_dir, 'feature_extraction_statistics.txt')
	with open(statistics_file, 'w') as f:
		f.write("WSI Feature Extraction Statistics\n")
		f.write("=" * 70 + "\n")
		...
		f.write(f"Feature Extraction Mode: Direct (No Normalization)\n")
